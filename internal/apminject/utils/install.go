...
// Download keeps a minimal compatible API.
// In this extracted project, launcher artifacts are expected to be prepared under install-dir manually.
func Download(log *Logger, opt ...Opt) error {
	var c config
	for _, fn := range opt {
		fn(&c)
	}
	...
	if c.pyLib {
		if log != nil {
			log.Infof("Installing ddtrace python library")
		}
		...
		if py != "" {
			...
			if py3Ver >= 7 {
				//nolint:gosec
				if s, err := exec.Command(py, "-m", "pip", "install", "ddtrace").CombinedOutput(); err != nil {
					if log != nil {
						log.Warn(string(s))
	...
	if c.launcherURL != "" && log != nil {
		log.Warnf("launcher download is not handled in minimal project, please prepare files under %s", filepath.Join(c.installDir, dirInject, dirInjectSubInject))
	}
	return nil
}

func Install(log *Logger, opt ...Opt) error {
	var c config
	for _, fn := range opt {
		fn(&c)
	}
	...
	if c.enableHostInject {
		...
		if hostVersion.LessThan(required) {
			if log != nil {
				log.Error(fmt.Errorf("host libc version %s is less than required %s", hostVersion, required))
			}
			if err := unsetPreload(c.installDir); err != nil {
				if log != nil {
					log.Error(err)
...
func Uninstall(opt ...Opt) error {
	...
	if err := unsetDockerRunc(dockerDaemonJSONPath); err != nil {
		_, _ = fmt.Fprintf(os.Stderr, "unset docker: %s\n", err)
	}

	if err := unsetPreload(c.installDir); err != nil {
		_, _ = fmt.Fprintf(os.Stderr, "unset preload: %s\n", err)
	}
...
func readPreloadWithoutLanucher(preloadPath, installDir string) (string, error) {
	...
	if err != nil {
		err = fmt.Errorf("failed to read %s: %w", preloadConfigFilePath, err)
		return "", err
	}
...
func unsetPreload(installDir string) error {
	lines, err := readPreloadWithoutLanucher(preloadConfigFilePath, installDir)
	if err != nil {
	...
	if err := os.WriteFile(preloadConfigFilePath, []byte(lines), 0o644); err != nil {
		return fmt.Errorf("failed to clean %s: %w", preloadConfigFilePath, err)
	}
	return nil
}
...
func setDockerRunc(configPath, runcPath string) error {
	...
	if cfgVal, _ := getDockerRuntimeInfoFromConfig(config); cfgVal != "" {
		if runcName != cfgVal {
			return fmt.Errorf("config not match the actual information: system info: %s, config: %s", runcName, cfgVal)
		} else {
			return nil
		}
	}
	...
	runtimes[RuntimeDkRunc] = map[string]any{"path": runcPath}
